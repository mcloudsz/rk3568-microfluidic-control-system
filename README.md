...
RK3568 微流控液滴视觉闭环控制系统。

基于 RK3568开发血液液滴混合闭环控制系统。控制血液、缓冲液两路泵阀生成液滴。采集液滴图像，基于 OpenCV 提取液滴混合状态，根据视觉结果调节两路泵阀输出。结合压力传感器和冲洗液泵阀实现超压事件上报和处理。
## 1. 项目简介
### 1.1 核心功能
系统主要实现以下功能：

- 通过 MIPI CSI 摄像头采集液滴图像；
- 使用 V4L2 和 DMA-BUF 管理主图像流采集缓冲区；
- 使用 RGA 完成 NV12 到 BGR 的图像格式转换；
- 分析液滴大小、颜色均值和颜色方差；
- 根据液滴大小过滤无效检测结果；
- 使用带前馈的 PI 控制调节液滴混合比例；
- 在保持两相流速比不变的情况下调节总流速，改善混合均匀度；
- 通过字符设备统一控制血液、缓冲液和冲洗液三路泵阀；
- 通过 ADS1115 阈值事件监测液路压力，并在超压时启动冲洗保护；
- 通过独立调试图像流按帧间隔保存灰度图像；
- 对可恢复业务异常执行复位，对不可恢复故障执行统一退出。
### 1.2 系统组成
系统由硬件设备、Linux 内核驱动和用户态应用三部分组成。

| 层级 | 模块 | 主要作用 |
|---|---|---|
| 内核驱动 | V4L2 摄像头链路 | 管理图像格式、采集队列和视频流 |
| 内核驱动 | RGA 与 DMA-BUF 接口 | 支持图像格式转换和缓冲区共享 |
| 内核驱动 | 泵阀字符设备驱动 | 统一管理三路泵阀的启动、停止和调频 |
| 内核驱动 | ADS1115 IIO 驱动 | 上报压力阈值事件 |
| 用户态应用 | 图像采集与转换模块 | 获取 NV12 图像并转换为 BGR |
| 用户态应用 | 液滴图像分析模块 | 计算液滴大小、颜色均值和颜色方差 |
| 用户态应用 | 液路闭环控制模块 | 执行比例环、均匀度环和 PI 冻结控制 |
| 用户态应用 | 压力监测模块 | 接收 IIO Event 并触发冲洗保护 |
| 用户态应用 | 调试图像保存模块 | 周期性保存灰度调试图像 |
### 1.3 系统运行流程

系统启动后按照以下流程运行：

1. 初始化全局状态、线程安全队列和图像缓冲池；
2. 打开 V4L2 图像节点、泵阀字符设备和 ADS1115 IIO 设备；
3. 配置图像格式和采集缓冲区，启动图像采集、处理、液路控制、压力监测和调试图像保存线程；
4. 启动血液与缓冲液两路泵阀，并根据目标混合比例设置前馈频率；
5. 图像采集线程获取 NV12 图像，RGA 将其转换为 BGR 后提交给图像分析线程；
6. 图像分析线程检测完整液滴，计算液滴大小、颜色均值和颜色方差；
7. 液路控制线程过滤大小异常的液滴，并通过比例环和均匀度环动态调节两路泵频率；
8. 每次更新泵速后暂时冻结 PI，等待新的控制作用到达视觉检测区域；
9. 压力超限或液滴连续异常时，停止正常液路并启动冲洗复位流程；
10. 设备访问失败或线程内部异常时，主线程停止各业务线程并统一释放系统资源。
## 2. 系统设计与关键实现
...
项目目录划分为用户态应用、内核驱动和项目说明三部分。各目录作用如下：

| 目录 | 主要内容 |
|---|---|
| `app/` | 摄像头采集、RGA 图像转换、液滴图像分析、液路闭环控制、压力监测和调试图像保存等用户态代码 |
| `kernel/` | 泵阀字符设备驱动、ADS1115 IIO 驱动 |
